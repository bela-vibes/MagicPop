...
export const PROJECTS: Project[] = [
  ...
  description: {
    de: "Taschen mit Haltung.\n\nFür LOQI haben wir Bildwelten für eine neue Generation vielseitiger Taschen entwickelt. Jede Tasche bringt eine eigene Persönlichkeit mit, die wir nicht überformen wollten, sondern sichtbar machen. Statt eines einheitlichen Looks haben wir mit wiederkehrenden Prinzipien gearbeitet. Klare Sets, präzise komponiert, die sich an die jeweilige Tasche anpassen. Materialität, Farbe und Kontext greifen ineinander und geben jeder Szene ihren eigenen Ton, ohne die Gesamtserie zu verlieren. Neben der visuellen Konzeption lag die gesamte technische Umsetzung bei uns. Von Casting über Location bis zum kompletten Produktionssetup. Kamera, Team und Ablauf wurden von uns aufgebaut und gesteuert. So entsteht eine Serie, die flexibel bleibt und trotzdem konsistent wirkt. Die Taschen stehen im Mittelpunkt, jedes für sich, und funktionieren gleichzeitig als Teil eines größeren Ganzen.",
    en: "Bags with character.\n\nFor LOQI, we developed a visual language for a new range of versatile bags. Each piece comes with its own personality, which we chose to reveal rather than override. Instead of forcing a single look, we worked with a set of recurring principles. Clear, composed sets that adapt to each bag. Materiality, color, and context interact to give every scene its own tone, while still holding the series together.\n\nAlongside the visual concept, we handled the full technical production. From casting and location to building the entire production setup, including camera, team, and workflow. The result is a series that remains flexible yet consistent. Each bag stands on its own, while contributing to a larger whole."
  },
