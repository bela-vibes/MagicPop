...
import { motion, AnimatePresence } from 'motion/react'; // AnimatePresence hinzugefügt
    ...
    <section id="projects" className="py-24 md:py-32 bg-transparent relative overflow-hidden transition-colors duration-500">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-4">
            {[null, ...categories].map((cat) => (
              <button
                ...
                className={`font-archivo text-xs uppercase tracking-widest relative pb-1 ${activeFilter === cat ? 'text-magic-black dark:text-off-white' : 'text-magic-black/40 dark:text-off-white/40'}`}
              >
                {cat === null ? (lang === 'de' ? 'Alle' : 'All') : cat}
      ...
      <div ref={scrollRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className={`flex overflow-x-auto gap-8 px-6 md:px-12 no-scrollbar pb-12 ${!isEdgeScrolling ? 'snap-x snap-mandatory' : ''}`}>
        {filteredProjects.map((project, index) => (
          <div key={project.id} onClick={() => setSelectedProject(project)} className="min-w-[80vw] md:min-w-[45vw] snap-start group cursor-pointer">
        ...
        {selectedProject && (
          ...
          >
            ...
            <div className="w-full h-20 bg-[#ff4d00] flex items-center justify-between px-6 md:px-12 z-[1010] shrink-0">
              ...
              <button onClick={handleCloseProject} className="text-white hover:rotate-90 transition-transform duration-300">
            ...
            <div className="flex-1 overflow-y-auto pt-12">
              <div className="max-w-7xl mx-auto px-6 md:px-12 pb-32">
                <h1 className="font-archivo text-5xl md:text-8xl uppercase tracking-tighter mb-16 text-magic-black dark:text-off-white">
                  {selectedProject.title[lang]}
                </h1>
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                  <div className="lg:col-span-7">
                    <p className="font-editorial text-2xl md:text-4xl italic mb-12 text-magic-black dark:text-off-white">
                      {selectedProject.description[lang].split('\n\n')[0]}
                    </p>
                    <img src={selectedProject.image} className="w-full h-auto rounded-sm" />
                  </div>
                  <div className="lg:col-span-5">
                     <p className="font-archivo text-base leading-relaxed text-magic-black/60 dark:text-off-white/60">
                      {selectedProject.description[lang].split('\n\n')[1]}
                    </p>
                  </div>
                </div>
                <div className="mt-24 flex flex-col items-center">
                  <button onClick={handleCloseProject} className="font-archivo uppercase tracking-widest text-xs text-magic-black/40">
                    {t.backToProjects}
                  </button>
                </div>
              </div>
